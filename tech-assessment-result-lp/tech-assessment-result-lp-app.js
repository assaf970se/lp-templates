function getParamData() {
  const MOCK_DATA =
    "";
  try {
    const strUrl = window.location.href;
    const url = new URL(strUrl);
    const encodedData = url.searchParams.get("ta") || MOCK_DATA;
    const strData = atob(encodedData);
    const data = JSON.parse(strData);
    return data;
  } catch (e) {
    const encodedData = MOCK_DATA;
    const strData = atob(encodedData);
    const data = JSON.parse(strData);
    return data;
  }
...
function renderSummaryContainerHtml(containerData, fullName, company) {
  const totalScore = containerData.reduce((acc, value) => acc + value.score, 0);
  function subjectScore(data, filter) {
    return data
      .filter((answer) => answer.subject.toLowerCase() === filter.toLowerCase())
...
async function init() {
  ...
  document.querySelector(".ta-result-summary-container").innerHTML =
    summaryContainerHtml;
  document.querySelector(".ta-result-details-containers-wrapper").innerHTML =
    DetailsContainerHtml;
  document.querySelector(".ta-result-details-header_title").innerHTML =
    renderDetailsHeaderTitleHtml(userFullData.fullName, totalScore);
  document.querySelector(".ta-result-details-header_subtitle").innerHTML =
    renderDetailsScoreSubtitle(totalScore);
  document.querySelector(".ta-loader-container").style = "display: none";
  document.querySelector(".ta-results-container").style = "display: grid";
}
